...
// Custom authentication check for API endpoints
// We need to return JSON errors instead of HTML redirects
session_start();
...
/**
 * API-specific authorization check (returns JSON instead of redirecting)
 */
function checkAPIAuthorization() {
    ...
    if (!isLocalhost($clientIP)) {
        // For non-localhost, could check whitelist here if needed
        // For now, we'll allow if IP check in auth_check.php would pass
    }
    
    // Check session authentication (if credentials exist)
    ...
    if (file_exists($credentialsFile) && filesize($credentialsFile) > 0) {
        ...
        if (!isset($_SESSION['authenticated']) || $_SESSION['authenticated'] !== true) {
            ...
            // Not authenticated - return JSON error
            echo json_encode([
                'success' => false,
...
// Perform API authorization check
