...
<head>
    ...
    <style>
        ...
        .actions-dropdown .dropdown-menu ul {
            list-style: none;
            margin: 0;
            padding: 0;
        }
        ...
        .actions-dropdown .dropdown-menu .menu-item:hover {
            background: var(--color-bg-hover);
            color: var(--color-primary);
        }

        .actions-dropdown .dropdown-menu .menu-item:disabled {
            opacity: 0.5;
            cursor: not-allowed;
        }
        ...
        /* Collapsible Database Tables Styles */
        ...
        .database-item-header:hover {
            background-color: var(--color-bg-hover);
        }

        .database-toggle-icon {
            margin-right: 8px;
            margin-top: 2px;
        }
        ...
        .collapsible-tables-header h4 {
            margin: 0;
            font-size: 16px;
            color: var(--color-primary);
        }

        .collapsible-tables-actions {
            display: flex;
            gap: 8px;
        }
        ...
        .collapsible-table-item {
            transition: background-color 0.2s ease;
        }

        .collapsible-table-item:hover {
            background-color: var(--color-bg-hover);
        }
        ...
        /* Database Actions Horizontal Alignment */
        .database-actions {
            display: flex;
            flex-direction: row;
            gap: 6px;
        ...
        /* Responsive adjustments for collapsible tables */
        @media (max-width: 768px) {
            .database-item-header {
                flex-direction: column;
                align-items: flex-start;
...
<body>
                        ...
                        <div class="actions-dropdown" id="statsActions">
                            ...
                            <div class="dropdown-menu" role="menu" aria-label="Database actions">
                                <ul>
                                    ...
                                    <li><a href="view_fixer.php" class="menu-item"
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <div id="columnsBuilder">
                        ...
                        <button type="button" class="btn-secondary" id="addColumnRowBtn"
                            style="margin-top: 8px; padding: 6px 10px; font-size: 12px;">➕ Add Column</button>
                    </div>
    ...
    <script>
            ...
            list.forEach(function (db) {
                ...
                const databaseItem = $(`
                    <div class="database-item ${isCurrent ? 'active' : ''}" data-database="${db.name}" tabindex="0">
                        <div class="database-item-header" onclick="toggleDatabaseTables('${db.name}')" style="cursor: pointer;">
                            <div class="database-toggle-icon">
                            ...
                            <div class="database-actions" style="display: flex; gap: 6px;">
                                <button class="btn-success" aria-label="Select ${db.name}" onclick="event.stopPropagation(); selectDatabase('${db.name}')" style="padding: 4px 8px; font-size: 11px;">Select</button>
                                <button class="btn-warning" aria-label="Export ${db.name}" onclick="event.stopPropagation(); openExportModal('${db.name}')" style="padding: 4px 8px; font-size: 11px;">Export</button>
                                <button class="btn-danger" aria-label="Delete ${db.name}" onclick="event.stopPropagation(); deleteDatabase('${db.name}')" style="padding: 4px 8px; font-size: 11px;">Delete</button>
                            </div>
                        </div>
                        <div class="database-tables-collapsible" id="tables-${db.name}" style="display: none;">
                            <div class="collapsible-tables-header">
                                <h4>📋 Tables in ${db.name}</h4>
                                <div class="collapsible-tables-actions">
                                    <button class="btn-secondary" onclick="refreshDatabaseTables('${db.name}')" style="padding: 4px 8px; font-size: 11px;">🔄 Refresh</button>
                                    <button class="btn-success" onclick="createTableInDatabase('${db.name}')" style="padding: 4px 8px; font-size: 11px;">➕ Create Table</button>
                                </div>
                            </div>
                            <div class="collapsible-tables-list" id="tables-list-${db.name}">
        ...
        function toggleDatabaseTables(databaseName) {
            const collapsible = $(`#tables-${databaseName}`);
            const arrow = $(`.database-item[data-database="${databaseName}"] .toggle-arrow`);
            ...
            if (collapsible.is(':visible')) {
                ...
                loadDatabaseTables(databaseName);
            }
        }

        // Load tables for a specific database
        function loadDatabaseTables(databaseName) {
            const tablesList = $(`#tables-list-${databaseName}`);

            // Show loading state
            tablesList.html('<div class="loading-tables">Loading tables...</div>');
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        displayDatabaseTables(databaseName, response.tables);
                    } else {
                        tablesList.html('<div class="error-message">Error loading tables: ' + (response.error || 'Unknown error') + '</div>');
                    }
                },
                error: function (xhr) {
                    tablesList.html('<div class="error-message">Error loading tables: ' + xhr.responseText + '</div>');
                }
            });
        }

        // Display tables for a specific database
        function displayDatabaseTables(databaseName, tables) {
            const tablesList = $(`#tables-list-${databaseName}`);
            tablesList.empty();
            ...
            if (tables.length === 0) {
                tablesList.append(`
                    <div class="empty-state" style="padding: 20px;">
                        ...
                        <p>This database doesn't have any tables yet.</p>
                    </div>
                `);
                return;
            }
            ...
            tables.forEach(function (table) {
                ...
                const tableItem = $(`
                    <div class="collapsible-table-item" data-table="${tableName}" style="cursor: pointer; padding: 8px 12px; border-bottom: 1px solid var(--color-border-lighter); display: flex; justify-content: space-between; align-items: center; transition: background-color 0.2s;">
                        <div class="table-info" style="display: flex; align-items: center; gap: 8px;">
                            <span class="table-icon">${tableIcon}</span>
                            <div>
                                <h5 style="margin: 0; font-size: 14px;">${tableName}${isView ? ' <span style="font-size: 11px; color: var(--color-warning);">(view)</span>' : ''}</h5>
                                <p style="margin: 0; font-size: 12px; color: var(--color-text-muted);">${typeLabel}</p>
                            </div>
                        </div>
                        <div class="table-actions" style="display: flex; gap: 4px;">
                            <button class="btn-success" onclick="event.stopPropagation(); viewTableFromDatabase('${tableName}', '${databaseName}')" style="padding: 3px 6px; font-size: 10px;">View</button>
                            ${!isView ? `<button class="btn-danger" onclick="event.stopPropagation(); deleteTableFromDatabase('${tableName}', '${databaseName}')" style="padding: 3px 6px; font-size: 10px;">Delete</button>` : ''}
                        </div>
                    </div>
                `);
                ...
                // Add hover effect
                tableItem.hover(
                    function () { $(this).css('background-color', 'var(--color-bg-hover)'); },
        ...
        // Refresh tables for a specific database
        function refreshDatabaseTables(databaseName) {
            loadDatabaseTables(databaseName);
        }

        // Create table in a specific database
        function createTableInDatabase(databaseName) {
            // Select the database first, then open create table modal
        ...
        // View table from a specific database
        function viewTableFromDatabase(tableName, databaseName) {
        ...
        // Delete table from a specific database
        function deleteTableFromDatabase(tableName, databaseName) {
            if (confirm(`Are you sure you want to delete table "${tableName}" from database "${databaseName}"?`)) {
